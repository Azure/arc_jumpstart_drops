...
* 2. [Omniverse Kit App Streaming Infrastructure Setup](#OmniverseKitAppStreamingInfrastructureSetup)
	...
	* 2.9. [API Management Service](#APIManagementService)
		...
		* 2.9.3. [*Add API route to ingress controller in APIM*](#AddAPIroutetoingresscontrollerinAPIM)
...
* 3. [Omniverse Kit App Streaming Deployment and Configuration](#OmniverseKitAppStreamingDeploymentandConfiguration)
	...
	* 3.12. [Helm Chart Deployment and Configuration](#HelmChartDeploymentandConfiguration)
		...
		* 3.12.3. [*ExternalDNS scripts external dns*](#ExternalDNSscriptsexternaldns)
		* 3.12.4. [*Create required secrets*](#Createrequiredsecrets)
	* 3.13. [Omniverse Kit App Streaming Services](#OmniverseKitAppStreamingServices)
		...
		* 3.13.3. [ *Deploy the custom streaming resources manifests omniverse azure*](#Deploythecustomstreamingresourcesmanifestsomniverseazure)
		* 3.13.4. [*Deploy HelmRepository manifests helm repostiories*](#DeployHelmRepositorymanifestshelmrepostiories)
		* 3.13.5. [*Create private DNS record for ingress controller*](#CreateprivateDNSrecordforingresscontroller)
		* 3.13.6. [*Create public DNS entry for App Gateway*](#CreatepublicDNSentryforAppGateway)
...
* 4. [ ISV Web App deployment](#ISVWebAppdeployment)
	* 4.1. [Deploying Web Client Application in Azure](#DeployingWebClientApplicationinAzure)
		* 4.1.1. [3D Viewport Hints](#DViewportHints)
		* 4.1.2. [3D Streaming Test](#DStreamingTest)
		* 4.1.3. [Configuration](#Configuration)
...
##  2. Omniverse Kit App Streaming Infrastructure Setup
...
####  2.6.4. *Add Subnets to the Virtual Network*
...
####  2.6.5. *Create Network Security Groups*
...
####  2.6.6. *Assign to subnets 'subnet-aks' and 'subnet-apim'* 
...
###  2.7. DNS and Certificate Configuration
...
####  2.7.1. *Create self-signed certificates for private DNS zone*
...
####  2.7.2. *Create LetsEncrypt certificates manually for public DNS zone*
...
####  2.7.3. *Create a .pfx from the full chain certificate and private key*
...
####  2.8.4. *Confirm and create cluster*
...
####  2.8.6. *Pull kubeconfig locally and check access*
...
###  2.9. API Management Service

####  2.9.1. *Create APIM Service*
...
####  2.9.2. *Configure Custom Domain and Private DNS Zone*
...
####  2.9.3. *Add API route to ingress controller in APIM*

1. Navigate to APIs under APIs in the APIM

    <img src="images/image65.png" style="width:9in;height:4in"/> 

2. Create a new API with the name HTTP

    <img src="images/image66.png" style="width:11in;height:6in"/> 

   1. Create a new GET operation under the HTTP API created in the last step
      * **URL**: `/*`
      * **Responses**:`200`

    <img src="images/image67.png" style="width:10in;height:5in"/> 
      ...
      1. Navigate to `Backend > HTTP(s) endpoint`
         ...
         <img src="images/image68.png" style="width:10in;height:5in"/> 

    2. Repeat this for `POST`, `DELETE` and `OPTIONS`

      <img src="images/image69.png" style="width:10in;height:6in"/>

3. Turn off Subscription Key Requirement
   1. Go to `APIM` > `APIs` > `APIs` > `http get` > 


###  2.10. Application Gateway Service with Web Application Firewall
...
##  3. Omniverse Kit App Streaming Deployment and Configuration
...
###  3.1. (Optional) Move External Dependencies to your own artifact storage
...
###  3.2. Download Sample and Resources Files
...
###  3.4. Create Image Registry Pull Secret
...
###  3.5. Install NVIDIA GPU Operator on AKS
...
###  3.8. Install Omniverse Resource Management Control Plane (RMCP) on AKS
...
###  3.9. Create and deploy a custom Omniverse Kit Application

####  3.9.1. Upload ISV custom kit app to a Container Registry
...
####  3.9.2. *Configure ISV Custom Kit App for Deployment*
...
###  3.10. Upload ISV custom kit app to a Container Registry
...
####  3.10.1. *Create Azure Container Registry using the Azure Portal*
...
###  3.11. Upload Helm Charts etc from NGC recommendation
...
###  3.12. Helm Chart Deployment and Configuration

####  3.12.1. *Set environment-specific values*
...
####  3.12.2. *Internal ingress controller helm nginx ingress controller*
...
####  3.12.3. *ExternalDNS scripts external dns*
...
####  3.12.4. *Create required secrets*
...
###  3.13. Omniverse Kit App Streaming Services

####  3.13.1. *Streaming helm kit appstreaming manager*
...
####  3.13.2. *Applications helm kit appstreaming applications*
...
####  3.13.3. *Deploy the custom streaming resources manifests omniverse azure*
...
####  3.13.4. *Deploy HelmRepository manifests helm repostiories*
...
####  3.13.5. *Create private DNS record for ingress controller*
...
####  3.13.6. *Create public DNS entry for App Gateway*
...
###  3.14. Validate Omniverse Kit App Streaming End Points via Swagger UI
...
###  4.1. Deploying Web Client Application in Azure
...
###  4.2. Power BI React Component Configuration

####  4.2.1. *Azure Apps Registration*
...
####  4.2.2. *Power BI Workspace Settings*
...
####  4.2.3. *Power BI Data Connections*
...
###  4.3. Event Hub React Component Configuration
...
###  4.4. Azure Static Web Application Deployment
...
####  4.4.1. *Deploying a React App to Azure Static Web Apps using Azure Portal*
